...
void ExistingTablesWindow::searchInDB()
{
    ...
    for (int i = 0; i < numThreads; i++)
    {
        ...
        searchTask->search(_column, _like, _typeSearch, _filter, _sort, limit, offset);
    }
}
...
{
    thread->completion(std::ref(model), _limit, offset, std::ref(_filter), std::ref(_sort));
}
...
void ExistingTablesWindow::refreshStartModel()
{
    ...
    _getMaxPageTread->getMaxPage(_rowsPerPage, _filter);
    initializationStartModel();
}
